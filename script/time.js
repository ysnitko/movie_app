...
async function renderMovieAbout(id) {
  ...
  <div class="characters">
    ...
    <button id="loadMoreCharacters" onclick=""></button>
    </div>
  </div>
  </div>
`;
...
async function renderMovieCharacters() {
  ...
  const charactersPromises = movie.characters.map((item) =>
    loadCharacterInfo(item)
  );
  const spinner = document.querySelector('.spinner');
  spinner.classList.add('show');
  // loadMoreButton.classList.add('hidden');
  let charactersList = await Promise.all(charactersPromises);
